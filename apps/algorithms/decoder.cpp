...
void readHuffmanTable(BitReader& bitReader, JPGImage* const image) {
    ...
    // count+=1;
    // if(count ==1){
    //     std::cout<<"0000000000000: "<<length<<'\n';
    // }
    while (length > 0) {
        byte tableInfo = bitReader.readByte();
        byte tableID = tableInfo & 0x0F;
        bool acTable = tableInfo >> 4;
        // std::cout<<" ==tableInfo==: "<<int(tableInfo)<<" ==tableID==: "<<int(tableID)<<"==acTAble==:"<<int(acTable)<<"\n";
        if (tableID > 3) {
        ...
        for (uint i = 0; i < allSymbols; ++i) {
            ...
            // std::cout<<int(hTable.symbols[i])<<'\n';
        }

        generateCodes(hTable);
        // for (uint i = 0; i < 16; ++i) {
        //     for (uint j = hTable.offsets[i]; j < hTable.offsets[i + 1]; ++j) {
        //         std::cout<<hTable.codes[j]<<'\n';
        //     }
        // }
        ...
        if (image->huffmanDCTables[i].set) {
            ...
            for (uint j = 0; j < 16; ++j) {
                ...
                for (uint k = image->huffmanDCTables[i].offsets[j]; k < image->huffmanDCTables[i].offsets[j + 1]; ++k) {
                    std::cout <<  (uint)image->huffmanDCTables[i].symbols[k] << std::dec << ' ';
                }
                std::cout << '\n';
            }
        }
        ...
        if (image->huffmanACTables[i].set) {
            ...
            for (uint j = 0; j < 16; ++j) {
                ...
                for (uint k = image->huffmanACTables[i].offsets[j]; k < image->huffmanACTables[i].offsets[j + 1]; ++k) {
                    std::cout << (uint)image->huffmanACTables[i].symbols[k] << std::dec << ' ';
                }
                std::cout << '\n';
            }
        }
...
void readScans(BitReader& bitReader, JPGImage* const image) {
    // decode first scan
    // byte a  = bitReader.readByte();
    // std::cout<<"begin: "<<int(a)<<'\n';
    // bitReader.back1Byte();

    readStartOfScan(bitReader, image);
    // a  = bitReader.readByte();
    // std::cout<<"after: "<<int(a)<<'\n';
    // bitReader.back1Byte();
    // printScanInfo(image);

    decodeHuffmanData(bitReader, image);
    // a  = bitReader.readByte();
    // std::cout<<"after2: "<<int(a)<<'\n';
    // bitReader.back1Byte();
    ...
    for (uint i = 0; i < 16; ++i) {
        ...
        for (uint j = hTable.offsets[i]; j < hTable.offsets[i + 1]; ++j) {
            if (currentCode == hTable.codes[j]) {
                // if(count<16){
                //     std:: cout<<"hTable: "<< hTable.symbols[j]<<'\n';
                // }
...
//   read from the BitReader
 #pragma region  decodeBlockComponent
